...
export default function handler(req, res) {
  ...
  if (!lobbyist && officialsList.length === 0) {
    res.status(400).json({
      error:
        "You must provide either an official, officials, or a lobbyist query parameter.",
    });
    return;
  }
  ...
  db.all(sql, params, (err, rows) => {
    ...
    if (officialsList.length > 0) {
      ...
      records = Object.entries(grouped).map(([lobbyist_name, dpo_entries]) => ({
        lobbyist_name,
        dpo_entries,
      }));
    } else {
      ...
      records = [
        {
          ...
          dpo_entries: rows.map(({ person_name, connection_count }) => ({
            person_name,
            connection_count,
          })),
        },
