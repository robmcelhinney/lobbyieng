...
export default function ConnectionsOfficial() {
  ...
  const visibleNodeIds = React.useMemo(() => {
    ...
    filteredLinks.forEach((link) => {
      ...
      if (typeof sourceId === "object" && sourceId !== null && "id" in sourceId)
    ...
    async function fetchLatestYearAndMethodAndData() {
      try {
        ...
        if (latestYear) {
          ...
          if (!res2.ok)
            throw new Error("Official not found or failed to fetch data");
  ...
  useEffect(() => {
    async function fetchFilters() {
      try {
        ...
        if (!res.ok)
          throw new Error("Official not found or failed to fetch filters");
        const data = await res.json();
        setYears(data.years || []);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        <header className="bg-blue-900 dark:bg-gray-800 text-white dark:text-gray-100 py-4">
          <div className="max-w-6xl mx-auto px-4 text-center">
            ...
            <p className="mt-1">
              Lobbyists connecting to {officialSlug.replace(/-/g, " ")}
            </p>
          </div>
        </header>
        <main className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-8">
          ...
          <div className="flex flex-col w-full relative" style={{ zIndex: 10 }}>
            <div className="mb-4 text-center relative" style={{ zIndex: 10 }}>
              <p>
                This force-directed graph shows the connections between
                lobbyists and {officialSlug.replace(/-/g, " ")}, based on real
                lobbying records.
              </p>
          ...
          >
            <div className="border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-md shadow-md p-4 mb-4 w-full max-w-2xl relative">
              ...
              <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                Click to filter. CTRL+Click to multi-select. Click again to
                reset.
              </div>
            </div>
          </div>
