...
import { buildCacheKey, getOrSetCache } from "./serverCache"

const execFileAsync = promisify(execFile)

// Contact details change rarely; cache per member URL for 24h. getOrSetCache
// also coalesces concurrent fetches for the same URL into one curl.
...
async function fetchContactsUncached(memberUrl) {
  let html = ""
  try {
    const { stdout } = await execFileAsync("curl", ["-L", "-s", "--max-time", String(CURL_MAX_TIME_SECONDS), memberUrl], {
      encoding: "utf-8",
...
export async function fetchOireachtasMemberContacts(memberUrl) {
  if (!memberUrl) return emptyContacts()
  const key = buildCacheKey("oireachtas-contacts", { url: memberUrl })
  const { value } = await getOrSetCache(key, CONTACTS_CACHE_TTL_MS, () => fetchContactsUncached(memberUrl))
  return value
}
