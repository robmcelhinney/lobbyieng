...
- `data/Lobbying_ie_returns_results*.csv` - Register of Lobbying CSV exports, fetched automatically via `npm run fetch:csvs` (or downloaded manually).
...
Recommended: fetch the Register CSV exports automatically, then ingest:

```bash
npm run refresh:data
```

This runs `scripts/fetch_lobbying_csvs.py` (downloads CSV exports from the
official Register API into `data/`) followed by `npm run build:db`. For a
quicker incremental refresh of recent returns only:

```bash
npm run refresh:data:current
```

Manual alternative (e.g. for a custom date range or filtered export):
...
```bash
uv run python scripts/download-oireachtas-images.py
```

Senator thumbnails are stored separately in `public/images/senator_thumbnails/`:

```bash
uv run python scripts/download-oireachtas-images.py --chamber seanad
```
...
1. Ensure Register CSV exports exist in `data/` (`npm run fetch:csvs`, or manual export).
