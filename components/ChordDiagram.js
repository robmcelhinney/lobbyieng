...
export default function ChordDiagram({
  ...
  useEffect(() => {
    ...
    if (!officials || officials.length === 0) {
      officials = Array.from(new Set(records.flatMap((r) => r.dpo_entries.map((d) => d.person_name))))
    }
    ...
    lobbyists.forEach((l, i) => {
      ...
      // official1 <-> lobbyist
      const entry1 = rec?.dpo_entries.find((d) => d.person_name === official1)
      matrix[0][i + 1] = entry1 ? entry1.connection_count || 1 : 0
      matrix[i + 1][0] = entry1 ? entry1.connection_count || 1 : 0
      // official2 <-> lobbyist
      const entry2 = rec?.dpo_entries.find((d) => d.person_name === official2)
      ...
      .on("mousemove", function (event, d) {
        ...
        if (d.index === 0 || d.index === labels.length - 1) {
          ...
          const total = records.reduce((sum, r) => {
            const entry = r.dpo_entries.find((e) => e.person_name === official)
            return sum + (entry ? entry.connection_count || 1 : 0)
          }, 0)
      ...
      {tooltip.visible && (
        <div
          className={`absolute text-sm rounded-md px-3 py-2 max-w-xs pointer-events-none z-50 shadow-lg transition-opacity duration-200 ${
            prefersDark ? "bg-gray-800 text-gray-100" : "bg-white text-gray-900 border border-gray-300"
          }`}
          style={{
            left: tooltip.x,
            top: tooltip.y
